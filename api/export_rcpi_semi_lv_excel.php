<?php
// RCPI - Semi-Expendable Low Value (unit_cost <= 5000) — Appendix 66 format
...
// Filter: Semi-Expendable AND unit_cost <= 5000
...
// Log
(new SystemLogModel())->log("EXPORT_RCPI_SEMI_LV", "Exported RCPI Semi-Expendable Low Value for $monthToReport");

// Headers
if (ob_get_level()) ob_end_clean();
$dateSuffix = $startDate && $endDate ? "{$startDate}_to_{$endDate}" : $monthToReport;
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment; filename=RCPI_SemiExpendable_LowValue_' . $dateSuffix . '.xls');
...
echo '<tr><td colspan="10" align="center" style="font-size:10pt;border:none;">(Semi-Expendable — Low Value &#8369; 5,000 and Below)</td></tr>';
...
foreach ($groupedSupplies as $cat => $items) {
    echo '<tr><td colspan="10" style="background-color:#e8f4ff;font-weight:bold;border:1px solid black;padding-left:5px;">' . htmlspecialchars($cat) . '</td></tr>';
