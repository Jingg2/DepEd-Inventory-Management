...
<head>
    ...
    <style>
        ...
        /* Supply Card Actions - Handled in dashboard.css */
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="stats">
            ...
            <div class="stat-card" id="stat-critical-stock" style="cursor: pointer; position: relative; overflow: hidden; border-top-color: #ff4500;">
                <i class="fas fa-exclamation-circle" style="position: absolute; right: -10px; bottom: -10px; font-size: 4rem; opacity: 0.1; color: #ff4500;"></i>
                <h3>Critical stocks Item</h3>
                <p><?php echo $criticalStockCount ?? 0; ?></p>
            </div>
        ...
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 20px;">
            ...
            <a href="<?php echo $root; ?>returned_items" class="returned-items-btn" style="background: #10b981; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: 600; text-decoration: none;">
                <i class="fas fa-recycle"></i> Returned Items Report
            </a>
                    ...
                    ?>
                        <div class="supply-card" data-id="<?php echo htmlspecialchars($supply['supply_id'] ?? $supply['id'] ?? ''); ?>" data-name="<?php echo htmlspecialchars($supply['item'] ?? ''); ?>" data-category="<?php echo htmlspecialchars($categoryName); ?>" data-quantity="<?php echo $qty; ?>" data-returned-quantity="<?php echo (int)($supply['returned_quantity'] ?? 0); ?>" data-stock-no="<?php echo htmlspecialchars($supply['stock_no'] ?? ''); ?>" data-unit="<?php echo htmlspecialchars($supply['unit'] ?? ''); ?>" data-description="<?php echo htmlspecialchars($supply['description'] ?? ''); ?>" data-unit-cost="<?php echo htmlspecialchars($supply['unit_cost'] ?? ''); ?>" data-total-cost="<?php echo htmlspecialchars($supply['total_cost'] ?? ''); ?>" data-status="<?php echo htmlspecialchars($supply['status'] ?? ''); ?>" data-image="<?php echo htmlspecialchars($supply['image_base64'] ?? $basePath . 'img/Bogo_City_logo.png'); ?>" data-property-classification="<?php echo htmlspecialchars($supply['property_classification'] ?? ''); ?>" data-low-threshold="<?php echo $lowThreshold; ?>" data-critical-threshold="<?php echo $criticalThreshold; ?>" data-previous-month="<?php echo (int)($supply['previous_month'] ?? 0); ?>" data-add-stock="<?php echo (int)($supply['add_stock'] ?? 0); ?>" data-issuance="<?php echo (int)($supply['issuance'] ?? 0); ?>" data-requisition="<?php echo (int)($supply['requisition'] ?? 0); ?>">
                            ...
                            <div class="qty-display qty-stock <?php echo $badgeClass; ?>">
                                ...
                                Stock: <span class="qty-value"><?php echo htmlspecialchars($supply['quantity'] ?? '0'); ?></span>
                            </div>
                            <?php if ((int)($supply['returned_quantity'] ?? 0) > 0): ?>
                            <div class="qty-display qty-returned">
                                Returned/Used: <span class="qty-value"><?php echo (int)$supply['returned_quantity']; ?></span>
                            </div>
                            <?php endif; ?>
                            <div class="actions">
