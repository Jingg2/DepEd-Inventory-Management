            ...
            if ($action === 'delete') {
                ...
                if ($id !== '') {
                    ...
                    if ($result) {
                        ...
                        header("Location: employees?success=delete");
                        exit();
                    } else {
                        $message = $this->model->lastError ?: "Failed to delete employee.";
                        if ($isAjax) {
                            if (ob_get_level()) ob_end_clean();
                            header('Content-Type: application/json');
                    ...
                    if (empty($errors) && $message === '') {
                        ...
                        if ($result) {
                            if ($isAjax) {
                                ...
                                $successType = ($action === 'update') ? 'update' : '1';
                                header("Location: employees?success=" . $successType);
                                exit();
                            }
                        } else {
                            if ($isAjax) {
